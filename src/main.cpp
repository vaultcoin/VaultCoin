...
uint256 hashGenesisBlock("0xc0f5abdd6118a24f616351cc3d88c88a8118fca25e27bf0dbdb530e3028aa917");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // vaultcoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1392212666; // Line: 2815
...
const string strMessageMagic = "vaultcoin Signed Message:\n";
...
{
    // vaultcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight < 10)
    {
        nSubsidy = 800000 * COIN; // First ten blocks is the pre mine of 9%
    }
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // vaultcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 35) { DiffMode = 2; } // vaultcoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 1000) { DiffMode = 2; }  //vaultcoin, Let early adopters have fun.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // vaultcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // vaultcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        */
        ...
        pchMessageStart[2] = 't';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0xfa75af3449000e5f1ddfcd8f8b872fe7c912da91ee61c50f44ae945dd1db03cd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce = 0;
        block.nTime = 1393942344;
        
        if (fTestNet)
        {

            block.nNonce = 759854;
            block.nTime = 1393947459;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // vaultcoin: increase each by adding 1 to bitcoin's value.
